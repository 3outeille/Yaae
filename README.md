...
- Yaae (Yet another autodiff engine) is an [automatic differentiation][automatic-diff] engine written in Numpy which comes with a small neural networks library. It supports scalar operation as well as tensors operations and comes with various functions such as exp, relu, sigmoid ... For educational puprose only.
